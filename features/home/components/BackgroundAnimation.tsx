  ...
  useEffect(() => {
    ...
    textureLoader.load(
      ...
      (error) => {
        ...
        // Fallback to simple white rectangles if texture fails to load
        createFallbackGhosts();
      }
    );

    const createGhostInstances = (texture: THREE.Texture) => {
      // Create multiple ghost instances using the PNG texture
      const numGhosts = 100;
      ...
      for (let i = 0; i < numGhosts; i++) {
        ...
        const material = new THREE.MeshBasicMaterial({
          ...
          alphaTest: 0.1, // This helps with PNG transparency
        });
    ...
    const createFallbackGhosts = () => {
      // Fallback: simple white rectangles if PNG fails to load
      const numGhosts = 100;
