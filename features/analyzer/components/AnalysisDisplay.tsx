...
interface AnalysisDisplayProps {
  ...
  onRefineSuggestion?: (suggestionText: string, suggestionTitle: string, index: number) => void;
  ...
  return (
    <div className="relative flex items-center justify-center w-48 h-48 font-mono">
       ...
       <svg className="w-full h-full" viewBox="0 0 120 120">
        {/* BG Lines */}
        <circle cx="60" cy="60" r="54" fill="none" stroke="rgba(0, 240, 255, 0.1)" strokeWidth="1" />
        <circle cx="60" cy="60" r="40" fill="none" stroke="rgba(0, 240, 255, 0.1)" strokeWidth="1" />

        {/* Meter */}
        <path
          d="M 30 90 A 42.42 42.42 0 1 1 90 90"
          strokeWidth="6"
        ...
        {[0, 25, 50, 75, 100].map(p => (
           <line key={p}
            ...
            strokeWidth="2"
            transform={`rotate(${p * 1.8 - 90} 60 60)`}
          />
        ))}
      </svg>
    </div>
  ...
  return (
    <div className="flex items-center">
      {[...Array(5)].map((_, index) => {
        ...
        if (score >= starValue - 0.5) {
          return (
            <div key={index} className="relative w-5 h-5">
              ...
              <svg className="w-5 h-5 text-slate-700 absolute" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path d={starPath} />
              </svg>
              {/* Filled star foreground, clipped */}
              <svg className="w-5 h-5 text-yellow-400 absolute" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" style={{ clipPath: 'inset(0 50% 0 0)' }}>
                <path d={starPath} />
              </svg>
            </div>
          );
        ...
        // Empty star
        return (
          <svg key={index} className="w-5 h-5 text-slate-700" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path d={starPath} />
          </svg>
        );
      })}
       <span className="ml-2 text-base font-medium text-slate-300 font-mono">({score.toFixed(1)}/5.0)</span>
    </div>
  );
};
...
const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({
  ...
  const [sortConfig, setSortConfig] = useState<{ key: 'score'; direction: 'ascending' | 'descending' } | null>(null);
  ...
  const getSortIndicator = (key: 'score') => {
    ...
    return sortConfig.direction === 'ascending' ? <span className="text-white ml-1">▲</span> : <span className="text-white ml-1">▼</span>;
  };
  ...
  return (
    <div className="mt-8 space-y-8 animate-fade-in">
      ...
      {onRefineSuggestion && (analysis?.improvementSuggestions || []).length > 0 && (
          <CollapsibleSection
          ...
          >
              <p className="text-slate-400 mb-6 text-base">{t('refineIdeaSubtitle')}</p>
              <div className="space-y-4">
                  {(analysis?.improvementSuggestions || []).map((suggestion, index) => (
                      <div key={index} className="bg-primary/50 p-4 border border-slate-700 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                          <div className="flex-grow">
                              <h3 className="font-bold text-lg text-accent">{suggestion.title}</h3>
                              <p className="text-slate-400 text-base mt-1">{suggestion.description}</p>
                          </div>
                          <button
                              onClick={() => onRefineSuggestion(suggestion.description, suggestion.title, index)}
      ...
      >
        <p className="text-base text-slate-400 whitespace-pre-wrap">{analysis.detailedSummary}</p>
      </CollapsibleSection>
          ...
          <table className="w-full text-left font-mono">
            <thead>
              <tr className="border-b border-slate-700 text-slate-400 uppercase text-sm tracking-wider">
                ...
                <th className="p-3 text-center cursor-pointer hover:text-white" onClick={() => requestSort('score')}>
                    {t('rubricScore')}
            ...
            <tbody>
              {sortedRubric.map((criterion, index) => (
                <tr key={index} className="border-b border-slate-800">
                  <td className="p-3 font-semibold text-slate-300 align-top">{t(criterionToLocaleKey[criterion.name] || criterion.name)}</td>
                  <td className="p-3 text-center align-top">
                    <StarRating score={criterion.score} />
                  </td>
                  <td className="p-3 text-slate-400 text-base align-top">{criterion.justification}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      >
        <div className="space-y-6">
          {(analysis?.founderQuestions || []).map((item, index) => (
            <div key={index} className="bg-primary/50 p-4 border-l-4 border-accent/50">
              <h3 className="font-bold text-lg text-slate-200">{item.question}</h3>
              <blockquote className="mt-2 pl-4 border-l-2 border-slate-700 italic text-base text-slate-400">
                "{item.ask}" - <span className="text-xs text-slate-500">({item.source})</span>
              </blockquote>
              <p className="mt-2 text-base text-slate-400"><strong className="text-slate-300">Why it matters:</strong> {item.why}</p>
              <div className="mt-3 pt-3 border-t border-slate-700">
                <p className="text-base text-slate-300 whitespace-pre-wrap">{item.analysis}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SWOTList title={t('swotStrengths')} items={analysis?.swotAnalysis?.strengths} color="text-green-400" />
          <SWOTList title={t('swotWeaknesses')} items={analysis?.swotAnalysis?.weaknesses} color="text-red-400" />
          <SWOTList title={t('swotOpportunities')} items={analysis?.swotAnalysis?.opportunities} color="text-blue-400" />
          <SWOTList title={t('swotThreats')} items={analysis?.swotAnalysis?.threats} color="text-yellow-400" />
        </div>
      </CollapsibleSection>
      ...
      >
        <div className="space-y-4">
          {(analysis?.currentMarketTrends || []).map((trend, index) => (
            <div key={index} className="bg-primary/50 p-4 border border-slate-700">
              <h3 className="font-bold text-lg text-accent">{trend.trend}</h3>
              <p className="text-base text-slate-400 mt-1">{renderTextWithLinks(trend.impact)}</p>
            </div>
          ))}
        </div>
      </CollapsibleSection>
          ...
          {(analysis?.competitors || []).map((competitor, index) => (
            <div key={index} className="bg-primary/50 p-4 border border-slate-700">
              <div className="flex justify-between items-start">
                <h3 className="font-bold text-lg text-accent">{competitor.name}</h3>
                {competitor.sourceLink && (
                  <a href={competitor.sourceLink} target="_blank" rel="noopener noreferrer" title={t('visitSourceTooltip')} className="text-slate-500 hover:text-accent transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
              ...
              <p className="text-base text-slate-400 mt-1">{competitor.description}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div>
                  <h4 className="font-semibold text-green-400 mb-2">{t('competitorStrengths')}</h4>
                  <ul className="list-disc list-inside text-slate-400 text-base space-y-1">
                    {(competitor.strengths || []).map((s, i) => <li key={i}>{s}</li>)}
                  </ul>
                </div>
                <div>
                  <h4 className="font-semibold text-red-400 mb-2">{t('competitorWeaknesses')}</h4>
                  <ul className="list-disc list-inside text-slate-400 text-base space-y-1">
        ...
        <div className="space-y-4">
          {(analysis?.monetizationStrategies || []).map((strategy, index) => (
            <div key={index} className="bg-primary/50 p-4 border border-slate-700">
              ...
              <p className="text-base text-slate-400 mt-1">{strategy.description}</p>
            </div>
          ))}
        </div>
      </CollapsibleSection>
         ...
         >
           <div className="space-y-4">
             {(analysis?.improvementSuggestions || []).map((suggestion, index) => (
               <div key={index} className="bg-primary/50 p-4 border border-slate-700">
                 <h3 className="font-bold text-lg text-accent">{suggestion.title}</h3>
                 <p className="text-base text-slate-400 mt-1">{suggestion.description}</p>
               </div>
             ))}
           </div>
         </CollapsibleSection>
      ...
      >
        <ol className="space-y-4">
          {(analysis?.nextSteps || []).map((step, index) => (
             <li key={index} className="flex items-start">
               <span className="flex-shrink-0 mr-4 h-8 w-8 rounded-full bg-accent/20 text-accent font-bold flex items-center justify-center font-mono">{index + 1}</span>
               <div>
                  <h3 className="font-bold text-lg text-slate-200">{step.title}</h3>
      ...
      <div className="mt-8 flex flex-col sm:flex-row justify-end items-center gap-4 animate-slide-in-up" style={{ animationDelay: '650ms' }}>
         {isLoggedIn && (
           isSaved ? (
             <>
                ...
                <button onClick={onGoToDashboard} className="flex items-center gap-2 px-3 py-2 text-sm font-medium uppercase tracking-wider text-slate-300 bg-primary/50 border border-slate-600 rounded-none hover:bg-accent/20 hover:text-accent hover:border-accent transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
             ...
             <button onClick={onSave} className="flex items-center gap-2 px-3 py-2 text-sm font-medium uppercase tracking-wider text-slate-300 bg-primary/50 border border-slate-600 rounded-none hover:bg-accent/20 hover:text-accent hover:border-accent transition-colors">
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
