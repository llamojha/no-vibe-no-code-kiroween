...
describe('GoogleAIAdapter Integration Tests', () => {
  ...
  describe('analyzeHackathonProject', () => {
    it('should successfully analyze hackathon project', async () => {
      // Arrange
      const projectDescription = 'An AI-powered tool that helps developers write better code';
      ...
      // Act
      const result = await adapter.analyzeHackathonProject(projectDescription, kiroUsage, category, locale);
    ...
    it('should handle hackathon analysis API errors', async () => {
      ...
      if (!result.success) {
        expect(result.error).toBeInstanceOf(AIServiceError);
        expect(result.error.message).toBe('Failed to analyze hackathon project with Google AI');
