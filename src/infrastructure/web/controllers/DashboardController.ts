...
export class DashboardController {
  ...
  async getDashboard(request: NextRequest): Promise<NextResponse> {
    try {
      ...
      const recentAnalysesDTOs: AnalysisResponseDTO[] =
   ...
   *
   * Supports two modes:
   * - optimized=true: Returns minimal data for dashboard cards (80-90% less data)
   * - optimized=false (default): Returns full analysis data
   */
    ...
    try {
      ...
      const sortBy =
        (url.searchParams.get("sortBy") as
          ...
          | "title") || "newest";
      const category =
        (url.searchParams.get("category") as "idea" | "kiroween" | "all") ||
        "all";
      const optimized = url.searchParams.get("optimized") === "true";

      const userId = UserId.fromString(authResult.userId);

      // Use optimized query for dashboard if requested
      if (optimized && !searchTerm) {
        const result = await this.getUserAnalysesUseCase.executeOptimized({
          userId,
          page,
          limit,
          sortBy,
      ...
      // Use the standard GetUserAnalysesUseCase for full data or search
      ...
      if (result.success) {
        const responseDTOs: AnalysisResponseDTO[] = result.data.analyses.map(
          (analysis) => ({
            id: analysis.id.value,
            idea: analysis.idea,
            score: analysis.score.value,
            detailedSummary:
              analysis.feedback || "No detailed summary available",
            criteria: analysis.suggestions.map((suggestion, index) => ({
              name: `Suggestion ${index + 1}`,
              score: 0, // Default score since suggestions don't have scores
              justification: suggestion,
