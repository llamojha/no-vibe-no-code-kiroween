...
describe('UserId Value Object', () => {
  ...
  describe('fromString', () => {
    ...
    it('should throw error for invalid UUID format', () => {
      ...
      invalidUUIDs.forEach(invalidUUID => {
        expect(() => UserId.fromString(invalidUUID))
          .toThrow(`Invalid UserId format: ${invalidUUID}. Must be a valid UUID.`);
      });
    });

    it('should handle UUID with different casing', () => {
      const upperCaseUUID = validUUID.toUpperCase();
      const id = UserId.fromString(upperCaseUUID);
  ...
  describe('reconstruct', () => {
    it('should reconstruct UserId from persistence', () => {
    ...
    it('should reconstruct without validation', () => {
      ...
      const id = UserId.reconstruct('invalid-but-from-persistence');
  ...
  describe('equals', () => {
    it('should return true for equal IDs', () => {
      const id1 = UserId.fromString(validUUID);
      const id2 = UserId.fromString(validUUID);
      expect(id1.equals(id2)).toBe(true);
    });

    it('should return false for different IDs', () => {
      const id1 = UserId.fromString(validUUID);
      const id2 = UserId.fromString(anotherValidUUID);
      expect(id1.equals(id2)).toBe(false);
    });

    it('should be case sensitive', () => {
      const id1 = UserId.fromString(validUUID);
      const id2 = UserId.fromString(validUUID.toUpperCase());
      expect(id1.equals(id2)).toBe(false);
    });
  });
