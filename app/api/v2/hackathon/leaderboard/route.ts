import { NextRequest, NextResponse } from 'next/server';
import { ServiceFactory } from '@/src/infrastructure/factories/ServiceFactory';
import { serverSupabase } from '@/lib/supabase/server';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

/**
 * Get hackathon leaderboard
 * GET /api/v2/hackathon/leaderboard
 */
export async function GET(request: NextRequest) {
  try {
    // Get Supabase client and create service factory
    const supabase = serverSupabase();
    const serviceFactory = ServiceFactory.getInstance(supabase);
    
    // Create hackathon controller
    const hackathonController = serviceFactory.createHackathonController();
    
    // Delegate to controller
    return await hackathonController.getLeaderboard(request);
  } catch (error) {
    console.error('V2 Hackathon Leaderboard API error', error);
    const message =
      error instanceof Error
        ? error.message
        : 'Failed to get hackathon leaderboard.';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
