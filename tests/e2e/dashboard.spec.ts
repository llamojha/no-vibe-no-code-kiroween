...
test.describe('Dashboard E2E Tests', () => {
   ...
   * Requirements: 4.4, 4.5
   */
  test('should display analysis history and allow viewing details', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      window.localStorage.setItem('saved_analyses', JSON.stringify([mockAnalysis]));
    });
  ...
  test('should display empty state when no data exists', async ({ page }) => {
    ...
    // Verify the "Your Analyses" heading is still visible
    const analysesHeading = page.locator('h2:has-text("Your Analyses")');
    await expect(analysesHeading).toBeVisible();
  ...
  test('should display Frankenstein ideas when available', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      window.localStorage.setItem('frankenstein_ideas', JSON.stringify([mockIdea]));
    });
