...
export class HackathonPage {
  ...
  constructor(page: Page) {
    ...
    this.languageSelect = page.locator('button[aria-label*="language"], button[aria-label*="idioma"]');
    this.analyzeButton = page.locator('button[type="submit"]');
    this.scoreElement = page.locator('text=/score|puntuación/i').first();
    this.categoryRecommendation = page.locator('text=/category|categoría|resurrection|frankenstein|skeleton|costume/i').first();
    this.summaryElement = page.locator('text=/summary|resumen|analysis|análisis/i').first();
    this.kiroUsageAnalysis = page.locator('text=/kiro.*usage|uso.*kiro/i').first();
    ...
    this.resultsContainer = page.locator('section[aria-labelledby="analysis-results-heading"]');
  }
  ...
  async enterKiroUsage(kiroUsage: string): Promise<void> {
    ...
    await this.descriptionInput.fill(`${currentValue}\n\nKiro Usage: ${kiroUsage}`);
  }
