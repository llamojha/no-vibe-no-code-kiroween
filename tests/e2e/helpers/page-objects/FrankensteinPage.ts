...
export class FrankensteinPage {
  ...
  constructor(page: Page) {
    ...
    this.modeSelector = page
      .locator(
        'button:has-text("Tech Companies"), button:has-text("AWS Services")'
      )
      .first();
    ...
    this.generateButton = page
      .locator(
        'button:has-text("Create Frankenstein"), button:has-text("Accept & Generate Idea")'
      )
      .first();
    this.ideaTitle = page
      .locator("h1")
      .filter({ hasText: /^(?!.*Doctor Frankenstein)/ })
      .first();
    this.ideaDescription = page
      .locator("text=Idea Description")
      .locator("..")
      .locator("div")
      .last();
    this.coreConcept = page
      .locator("text=Core Concept")
      .locator("..")
      .locator("div")
      .last();
    this.problemStatement = page
      .locator("text=Problem Statement")
      .locator("..")
      .locator("div")
      .last();
    this.proposedSolution = page
      .locator("text=Proposed Solution")
      .locator("..")
      .locator("div")
      .last();
    this.metricsSection = page
      .locator("text=Originality, text=Feasibility, text=Impact")
      .first();
    this.slotMachineAnimation = page
      .locator('.slot-machine, [class*="slot"]')
      .first();
    this.loadingSpinner = page
      .locator("text=Bringing your Frankenstein to life, text=Loading")
      .first();
  ...
  async navigate(): Promise<void> {
    ...
    await this.page.locator('h1:has-text("Doctor Frankenstein")').waitFor({
      state: "visible",
      timeout: 60000,
    });
  }
  ...
  async selectLanguage(language: "en" | "es"): Promise<void> {
    ...
    const currentLang = await this.page
      .locator('[data-testid="language-toggle"]')
      .textContent();
    const needsChange =
      (language === "en" && currentLang?.includes("ES")) ||
  ...
  async addMultipleElements(elements: string[]): Promise<void> {
    // Not used in actual workflow - elements are randomly selected
    console.warn(
      "addMultipleElements is not used in Doctor Frankenstein workflow"
    );
  }
  ...
  async clickGenerate(): Promise<void> {
    ...
    const createButton = this.page.locator(
      'button:has-text("Create Frankenstein")'
    );
    if (await createButton.isVisible()) {
      ...
      const acceptButton = this.page.locator(
        'button:has-text("Accept & Generate Idea")'
      );
  ...
  async waitForSlotMachineAnimation(): Promise<void> {
    try {
      await this.slotMachineAnimation.waitFor({
        state: "visible",
        timeout: 5000,
      });
      await this.slotMachineAnimation.waitFor({
  ...
  async getAddedElements(): Promise<string[]> {
    const items = await this.elementsList
